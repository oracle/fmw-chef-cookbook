...
#
# wls provider for linux
provides :fmw_wls_wls, os: 'linux' if respond_to?(:provides)
...
def load_current_resource
  Chef::Log.info('wls provider, wls_linux load current resource')
  @current_resource ||= Chef::ResourceResolver.resolve('fmw_wls_wls_linux').new(new_resource.name)
...
action :install do
  ...
  if @current_resource.exists
    ...
    converge_by("Create resource #{ @new_resource }") do
      ...
      directory middleware_home_dir do
        mode   0775
        owner  os_user
        group  os_group
        action :create
