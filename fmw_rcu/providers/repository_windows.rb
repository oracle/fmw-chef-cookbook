...
def load_current_resource
  ...
  @current_resource ||= Chef::ResourceResolver.resolve('fmw_rcu_repository_windows').new(new_resource.name)
  ...
  if @new_resource.version == '10.3.6'
    wlst_utility = "#{@new_resource.middleware_home_dir}\\wlserver_10.3\\common\\bin\\wlst.cmd"
  else
    wlst_utility = "#{@new_resource.middleware_home_dir}\\oracle_common\\common\\bin\\wlst.cmd"
  end

  shell_out!("#{wlst_utility} #{@new_resource.tmp_dir}\\checkrcu.py #{@new_resource.jdbc_connect_url} #{@new_resource.db_connect_password} #{@new_resource.rcu_prefix} #{@new_resource.db_connect_user}").stdout.each_line do |line|
    unless line.nil?
  ...
  if @current_resource.exists
    ...
    converge_by("Create resource #{ @new_resource }") do
      ...
      execute "Create #{new_resource.rcu_prefix}" do
        command "#{new_resource.oracle_home_dir}\\bin\\rcu.bat -silent -createRepository -databaseType ORACLE -connectString #{new_resource.db_connect_url} -dbUser #{new_resource.db_connect_user} -dbRole SYSDBA  -schemaPrefix #{new_resource.rcu_prefix} #{components_string} -f < #{tmp_file.path}"
...
action :drop do
  ...
  if @current_resource.exists
    converge_by("Delete resource #{ @new_resource }") do
      ...
      execute "Create #{new_resource.rcu_prefix}" do
        command "#{new_resource.oracle_home_dir}\\bin\\rcu.bat -silent -dropRepository -databaseType ORACLE -connectString #{new_resource.db_connect_url} -dbUser #{new_resource.db_connect_user} -dbRole SYSDBA  -schemaPrefix #{new_resource.rcu_prefix} #{components_string} -f < #{tmp_file.path}"
        cwd new_resource.tmp_dir
        environment('JAVA_HOME' => new_resource.java_home_dir)
      end
