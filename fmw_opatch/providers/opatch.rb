...
def load_current_resource
  ...
  shell_out!("su - #{@new_resource.os_user} -c '#{@new_resource.oracle_home_dir}/OPatch/opatch lsinventory -patch_id -oh #{@new_resource.oracle_home_dir} -invPtrLoc #{@new_resource.orainst_dir}/oraInst.loc'").stdout.each_line do |line|
    unless line.nil?
...
action :apply do
  ...
  if @current_resource.exists
    ...
    converge_by("Create resource #{ @new_resource }") do
      result = false
      shell_out!("su - #{@new_resource.os_user} -c '#{@new_resource.oracle_home_dir}/OPatch/opatch apply -silent -jre #{@new_resource.java_home_dir}/jre -oh #{@new_resource.oracle_home_dir} #{@new_resource.tmp_dir}/#{@new_resource.patch_id}'", :timeout => 1200).stdout.each_line do |line|
        unless line.nil?
          Chef::Log.info(line)
...
# opatch rollback on a unix host
action :rollback do
  Chef::Log.info("#{@new_resource} fired the rollback action")
  if @current_resource.exists
    converge_by("Rollback resource #{ @new_resource }") do
      result = false
      shell_out!("su - #{@new_resource.os_user} -c '#{@new_resource.oracle_home_dir}/OPatch/opatch rollback -id #{@new_resource.patch_id} -silent -jre #{@new_resource.java_home_dir}/jre -oh #{@new_resource.oracle_home_dir} '", :timeout => 1200).stdout.each_line do |line|
        unless line.nil?
          Chef::Log.info(line)
          if line.include? 'OPatch completed' or line.include? 'OPatch succeeded'
            result = true
