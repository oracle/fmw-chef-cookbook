...
#
# Adminserver control on a solaris host
provides :fmw_domain_adminserver, os: 'solaris2' if respond_to?(:provides)
...
def load_current_resource
  Chef::Log.info('adminserver provider, adminserver_linux provider load current resource')
  @current_resource ||= Chef::ResourceResolver.resolve('fmw_domain_adminserver_solaris').new(new_resource.name)
  ...
  output = DomainHelper.server_status(@new_resource.os_user, @new_resource.adminserver_name, @new_resource.weblogic_home_dir, @new_resource.weblogic_user, @new_resource.weblogic_password, @new_resource.nodemanager_listen_address, @new_resource.nodemanager_port, @new_resource.domain_name, @new_resource.domain_dir)
